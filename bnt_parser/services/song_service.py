...
class SongService:
    ...
    def __init__(self, table_service: TableService, genius_client: GeniusClient):
    ...
    def save_song(self):
        ...
        """
        if not self.lyrics or not self.title or not self.artist:
            raise ValueError("Incomplete song data. Cannot save song.")

        # Save the album
        if self.genius_record["album"] is not None:
            album_entry = self.genius_client.fetch_entry(
                path=self.genius_record["album"]["api_path"]
            )
    ...
    def parse_words(self, line: str) -> list[str]:
        ...
        for word in words:
            ...
            # Detect possessives; save the base word in addition to the possessive form
