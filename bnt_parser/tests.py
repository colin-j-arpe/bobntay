...
class SongServiceTestCase(TestCase):
    ...
    def test_save_song(self):
        ...
        assert str(context.exception) == "Incomplete song data. Cannot save song.", (
            "Cannot save without title, artist, and lyrics"
        )
    ...
    def test_parse_sections(self):
        ...
        assert self.service.sections == expected_sections, (
            "Parsed sections do not match expected structure"
        )
...
class TableServiceTestCase(TestCase):
    ...
    def test_get_table(self):
        ...
        assert str(context.exception) == "Table 'invalid_table' is not recognised.", (
...
class SongTableTestCase(TestCase):
    ...
    def test_find_song_with_release(self):
        ...
        self._make_song(
            external_id=2, title="Echos Myron", artist="Guided by Voices", release=release
        )
        result = self.table.find_song(
            title="Echos Myron",
            artist="Guided by Voices",
    ...
    def test_song_exists_false(self):
        assert (
            self.table.song_exists(title="Nonexistent", artist="Nobody", release_title=None)
            is False
        )
    ...
    def test_save_if_not_exists_existing_song(self):
        ...
        assert (
            ...
            == 1
        ), "Should not create a duplicate song"
