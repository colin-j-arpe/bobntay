...
def _make_word(text, song=None):
    ...
    if song:
        section = Section.objects.create(song=song, order=1, type=Section.SectionTypeEnum.VERSE)
...
class FetchInflectionsTestCase(TestCase):
    ...
    def test_returns_inflections_from_entries(self):
        ...
        with (
            ...
            patch(
                "bnt_searcher.clients.mw_client.requests.get",
    ...
    def test_returns_empty_when_response_is_empty(self):
        with (
            patch.dict("os.environ", {"MW_API_KEY": "test-key"}),
            patch(
                "bnt_searcher.clients.mw_client.requests.get", return_value=self._mock_response([])
            ),
        ):
    ...
    def test_returns_empty_on_http_error(self):
        ...
        with (
            patch.dict("os.environ", {"MW_API_KEY": "test-key"}),
            patch("bnt_searcher.clients.mw_client.requests.get", return_value=mock_resp),
        ):
            result = fetch_inflections("run")
...
class GetVariantsTestCase(TestCase):
    def test_alias_hit_returns_variants_without_api_call(self):
        lookup = WordVariantLookup.objects.create(headword="run", fetched_at=datetime.now(tz=UTC))
    ...
    def test_alias_miss_headword_exists_creates_alias_no_new_lookup(self):
        ...
        with patch(
            "bnt_searcher.services.variant_service.fetch_inflections", return_value=("run", ["ran"])
        ):
            result = get_variants("run")
    ...
    def test_alias_miss_no_lookup_creates_lookup_variants_alias(self):
        with patch(
            "bnt_searcher.services.variant_service.fetch_inflections",
            return_value=("run", ["ran", "running"]),
        ):
            result = get_variants("running")

        lookup = WordVariantLookup.objects.get(headword="run")
        assert set(WordVariant.objects.filter(lookup=lookup).values_list("text", flat=True)) == {
            "ran",
            "running",
        }
    ...
    def test_none_headword_falls_back_to_search_term_as_headword(self):
        with patch(
            "bnt_searcher.services.variant_service.fetch_inflections", return_value=(None, [])
        ):
...
class WriterListViewTestCase(TestCase):
    def setUp(self):
        self._patch = patch(
            "bnt_searcher.views.PRIMARY_WRITERS", ["Robert Pollard", "Taylor Swift"]
        )
        self._patch.start()
